...
class ExpectedSarsa(basic.WithoutModel):
    def __init__(self, obstacle_reward, final_reward, loop_time=100000, alpha=0.5, gamma=0.5, epsilon=0.5):
        super(ExpectedSarsa, self).__init__(obstacle_reward, final_reward, loop_time, epsilon)
        self.alpha = alpha
        self.gamma = gamma
            ...
            if False:
                ...
                while True:
                    ...
                    if not self.is_obstacle(state_):
                        ...
                        p = self.policy_probability(state_, 'epsilon_greedy')
                        sum_ = 0
                        for i in range(4):
                            sum_ += self.Q[state_[0], state_[1], i]*p[i]
                        tmp = self.gamma * sum_ - self.Q[state[0], state[1], action]
                        # print(tmp, reward)
...
if __name__ == "__main__":
    agent = ExpectedSarsa(-100, 500, 500, 0.5, 0.9, 0.5)
    agent.loop()
